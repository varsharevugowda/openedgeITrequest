...
DO:
        ...
        do:
            ...
            END.
            MESSAGE "added user" objAdd:usrNum "sucessfuly"
                VIEW-AS ALERT-BOX.
        END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE adduser gDialog
PROCEDURE adduser:
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/
DEFINE VARIABLE objusr as businesslogic.ItRequest no-undo.
objusr = new businesslogic.ItRequest().
    
empty TEMP-TABLE ttUser.

CREATE ttUser.
assign
ttUser.ttfirstName  = filName:SCREEN-VALUE in frame {&frame-name}
ttUser.ttlastName  = filLname:SCREEN-VALUE.

objusr:adduserexp(input table ttUser).
  ...
  FIND FIRST ttUser WHERE ttUser.ttusrid = inum NO-LOCK NO-ERROR.
    ...
    DO:
        DO WITH FRAME {&frame-name} :
            ...
            ASSIGN
                filName:SCREEN-value  = STRING(ttUser.ttfirstName)
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE updateuser gDialog
