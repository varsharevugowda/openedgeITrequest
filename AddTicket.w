...
ON ENTRY OF cbTicketCat IN FRAME gDialog /* Ticket Category */
DO:
  
        FOR EACH ticket NO-LOCK:

        IF LOOKUP (Ticket.category , cbTicketCat:LIST-ITEMS) = 0 OR cbTicketCat:num-items  = 0  THEN 
         cbTicketCat:ADD-LAST (Ticket.category).
                      end.
         
         FOR EACH UserTicket NO-LOCK:
             IF LOOKUP (UserTicket.ticketstatus , cbTicketStatus:LIST-ITEMS) = 0 OR cbTicketStatus:num-items  = 0  THEN 
         cbTicketStatus:ADD-LAST (UserTicket.ticketstatus).
             end.
END.
