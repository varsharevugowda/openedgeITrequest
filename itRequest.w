...
DEFINE VARIABLE objinitalvalue AS businesslogic.ItRequest.
...
&Scoped-define FIELDS-IN-QUERY-brwuserTicket ttuserTicket.toggle ttuserTicket.tusrid ttuserTicket.tusrTicketId ttuserTicket.tassigne ttuserTicket.tstartdate ttuserTicket.tenddate ttuserTicket.tticketId ttuserTicket.tticketStatus ttuserTicket.tTicketDiscription ttuserTicket.tcategory /*   
&Scoped-define ENABLED-FIELDS-IN-QUERY-brwuserTicket ALL WITH FRAME brwuserTicket*/   ENABLE ttuserTicket.tticketStatus   /*  ENABLE ALL WITH FRAME fTable*/   
...
filPhone btnUpdateusr bundo btnreport bselect bdeselect brwuserTicket ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brwuserTicket C-Win _FREEFORM
  QUERY brwuserTicket DISPLAY
      ttuserTicket.toggle COLUMN-LABEL "select" VIEW-as toggle-box
    ttuserTicket.tusrid column-LABEL "Userid"  
...
/*    ENABLE ALL WITH FRAME brwuserTicket*/
    ENABLE ttuserTicket.tticketStatus
    
/*    ENABLE ALL WITH FRAME fTable*/
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     ...
     btnUpdateusr AT ROW 9.33 COL 128 WIDGET-ID 202
     bundo AT ROW 9.57 COL 95 WIDGET-ID 220
     btnreport AT ROW 9.57 COL 107 WIDGET-ID 218
     ...
     btnUpdate AT ROW 13.86 COL 129 WIDGET-ID 20
     ...
     "USER INFORMATION" VIEW-AS TEXT
          ...
          FGCOLOR 0 FONT 6
     RECT-1 AT ROW 2.43 COL 5 WIDGET-ID 16
...
DO:
        ...
        if brwuserTicket:SELECT-ALL THEN
        do:
            brwuserTicket:DESELECT-ROWS().
            bselect:CHECKED = FALSE.
            hbrowser = brwuserTicket:HANDLE.
            hcolumn = hbrowser:GET-BROWSE-COLUMN(1).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL brwuserTicket C-Win
ON VALUE-CHANGED OF brwuserTicket IN FRAME DEFAULT-FRAME
DO:
        DEFINE VARIABLE hbrowser AS HANDLE NO-UNDO.
        DEFINE VARIABLE hcolumn  AS HANDLE NO-UNDO.
...
DO:
        ...
        brwuserTicket:DESELECT-ROWS().
        MESSAGE NUM-RESULTS("brwuserticket")
            VIEW-AS ALERT-BOX.
        DO intSub = 1 TO NUM-RESULTS("brwuserticket"):
            reposition brwuserTicket to ROW(intSub).
            hbrowser = brwuserTicket:HANDLE.        
            hcolumn = hbrowser:GET-BROWSE-COLUMN(1).
            hcolumn:CHECKED = TRUE.                   
            brwuserTicket:SELECT-FOCUSED-ROW().
            bdeselect:CHECKED = false.
...
DO:
        ...
        if bselect:CHECKED = TRUE THEN
        DO:      
            for each ttuserTicket.              
...
DO:
        ...
        if(filUserId:SCREEN-VALUE ="")OR(filUserId:SCREEN-VALUE = "0") THEN
        DO:
            MESSAGE "plz enter correct userid"
                VIEW-AS ALERT-BOX.
...
/*        iNum = ttUser.ttusrid.*/
/*        RUN ui/AddUser.w(input-output iNum,input-output TABLE ttuser).*/
...
DO:
        DEFINE VARIABLE objundo AS businesslogic.ItRequest NO-UNDO.
        objundo = NEW businesslogic.ItRequest().
        objundo:getUndoinginfo( input int(filUserId:SCREEN-VALUE),INPUT TABLE ttUserundo).  
        APPLY "choose" to btnSearch.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    empty temp-table ttuserundo.
    objinitalvalue = new businesslogic.ItRequest().
    objinitalvalue:UndoUser(output table ttuserundo).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 RECT-4 btnAdTr filUserId btnSearch btnAdSearch BtnUpUsr 
         fillFName filLName filEmail btnDeleteUsr filPhone btnUpdateusr bundo 
         btnreport bselect bdeselect brwuserTicket btnAddUsrTicket btnUpdate 
         btnDelete 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
